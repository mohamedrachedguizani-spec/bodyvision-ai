...
class BodyAnalysisEngine:
    ...
    def adjust_body_fat_based_on_yolo(self, body_fat_percent: float, body_fat_kg: float, 
        ...
        if yolo_class:
            ...
            if "musculaire" in yolo_class_lower:
                ...
                # YOLO détecte "Moyenne" => ajuster vers les plages normales selon sexe et IMC
                target_range = self._get_normal_fat_range(sex, bmi)
                adjusted_percent = self._clamp_to_target_range(body_fat_percent, target_range)
                adjusted_kg = weight * (adjusted_percent / 100)
    ...
    def _detect_specific_posture_issues(self, landmarks) -> List[Dict[str, Any]]:
        """Redirige vers la détection frontale (éviter la duplication de code)"""
        return self._detect_front_view_posture_issues(landmarks)
    ...
    def analyze_body_composition(self, user_data: Dict[str, Any], yolo_classification: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if weight and height and age and sex and bmi:
            try:
                ...
                # 6. Validation de cohérence finale
                # S'assurer que la somme des composants ne dépasse pas le poids total
                if body_fat_kg and lean_body_mass and (body_fat_kg + lean_body_mass) > weight * 1.05:
                    # Recalibrer proportionnellement
                    total = body_fat_kg + lean_body_mass
                    body_fat_kg = body_fat_kg * (weight / total)
                    lean_body_mass = lean_body_mass * (weight / total)
                    body_fat_percent = (body_fat_kg / weight) * 100
                    skeletal_muscle_mass = min(skeletal_muscle_mass, lean_body_mass * 0.55)
        ...
        return {
            ...
            "age": age,
            "sex": sex,
            "body_water_estimated": self._estimate_body_water(weight, sex, age) if weight else None,
            "bone_mass_estimated": self._estimate_bone_mass(weight, height, sex) if weight else None,
    ...
    def _get_normal_fat_range(self, sex: str, bmi: float) -> Tuple[float, float]:
        """Retourne la plage normale de graisse corporelle selon le sexe et l'IMC"""
        if sex.lower() == 'male':
            if bmi < 22:
                return (12, 18)
            elif bmi < 25:
    ...
    def _clamp_to_target_range(self, body_fat_percent: float, target_range: Tuple[float, float]) -> float:
        """Ajuste progressivement la valeur vers la plage cible sans forcer"""
        low, high = target_range
        if body_fat_percent > high:
            # Ramener progressivement vers la plage haute (facteur 0.75)
            return max(low, min(high, body_fat_percent * 0.75))
        elif body_fat_percent < low:
            # Remonter progressivement vers la plage basse (facteur 1.2)
            return max(low, min(high, body_fat_percent * 1.2))
        else:
            return body_fat_percent  # Déjà dans la plage, on garde
    
    def _estimate_visceral_fat(self, body_fat_percent: float, sex: str, age: int) -> float:
        """Estime le niveau de graisse viscérale (échelle 1-20) basé sur la graisse totale, le sexe et l'âge
        
        Échelle standard utilisée par les balances impédancemétriques :
        1-9 : Normal (sain)
        10-14 : Élevé (risque modéré)
        15+ : Très élevé (risque important)
        """
        if not body_fat_percent:
            return None
        
        # Base : estimation à partir du % de graisse et du sexe
        if sex.lower() == 'male':
            # Les hommes stockent plus de graisse viscérale
            base_visceral = (body_fat_percent - 10) * 0.5 + 3
        else:
            # Les femmes stockent moins de graisse viscérale (plus sous-cutanée)
            base_visceral = (body_fat_percent - 15) * 0.35 + 2
        
        # Ajustement pour l'âge (la graisse viscérale augmente avec l'âge)
        if age < 30:
            age_factor = 0.85
        elif age < 40:
            age_factor = 1.0
        ...
        visceral_level = base_visceral * age_factor
        
        # Clamp entre 1 et 20
        visceral_level = max(1, min(20, visceral_level))
        
        return round(visceral_level, 1)
    
    def _estimate_body_water(self, weight: float, sex: str, age: int) -> float:
        """Estime l'eau corporelle totale en kg
        
        Hommes : ~60% du poids corporel (diminue avec l'âge)
        Femmes : ~50-55% du poids corporel (diminue avec l'âge)
        Référence : Watson formula simplifiée
        """
        if not weight:
            return None
        ...
        # Ajustement pour l'âge (l'eau corporelle diminue avec l'âge)
        if age and age > 50:
            base_ratio -= 0.03
        elif age and age > 65:
            base_ratio -= 0.05
        
        return round(weight * base_ratio, 1)
    
    def _estimate_bone_mass(self, weight: float, height: float, sex: str) -> float:
        """Estime la masse osseuse en kg
        
        Formule simplifiée basée sur le poids et la taille :
        Hommes : ~15% de la masse maigre, environ 3-5% du poids total
        Femmes : ~12% de la masse maigre, environ 2.5-4% du poids total
        Référence : Heymsfield et al.
        """
        if not weight:
            return None
        
        if sex.lower() == 'male':
            # Hommes : masse osseuse plus élevée
            if weight < 65:
                bone_mass = 2.5
            elif weight < 95:
                bone_mass = 3.2
            ...
            if height and height > 180:
                bone_mass += 0.3
            elif height and height < 165:
                bone_mass -= 0.2
        else:
            # Femmes : masse osseuse plus faible
            if weight < 50:
                bone_mass = 1.8
            elif weight < 75:
                bone_mass = 2.4
                ...
                bone_mass = 2.9
            # Ajustement par taille
            if height and height > 170:
                bone_mass += 0.2
            elif height and height < 155:
                bone_mass -= 0.2
